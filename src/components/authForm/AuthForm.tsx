    ...
    >
      <section className={classes.mainMobile}>
        <div className={cx(classes.columnFlex, classes.contents)}>
          <div
            className={cx(classes.rowFlex, classes.biLogo)}
            onClick={() => navigate("/")}
          >
            <img
          ...
          <div className={cx(classes.columnFlex, classes.formBoxArea)}>
            <FormInput
              value={form.getInputProps("email")}
              placeholder="이메일"
            />
            <FormInput
          ...
          <div className={classes.checkBoxArea}>
            {["(필수) 슈퍼코딩 이용약관"].map((message, index) => (
              <div
                key={index}
                className={cx(classes.rowFlex, classes.agreements)}
              >
                <Checkbox
                ...
                <div id="agreement_content">
                  ...
                  <div id="agreement_modalBtn" onClick={() => {}}>
                    내용보기
                  </div>
                </div>
              </div>
      ...
      <div className={cx(classes.columnFlex, classes.contents)}>
        ...
        <div style={{ marginTop: "31px" }}>
          <Button
            ...
            onClick={() =>
              loginHandler(form.values.email, form.values.password)
            }
          >
            로그인하기
          </Button>
...
export const AuthForm = ({ modalOpen, authType }: AuthFormModalProps) => {
  ...
  const [, setLogin] = useLocalStorage<IUserProfile | null>({
    key: "login",
    defaultValue: null,
  });
  const [, setAuthorized] = useLocalStorage<string | null>({
    key: "authorization",
    defaultValue: null,
  });
  const [, setRegistered] = useLocalStorage<boolean>({
    key: "registered",
    defaultValue: false,
  });
  ...
  const form = useForm<{
    ...
    validationRules: {
      ...
      confirmPassword: (val, values) =>
        authType === "로그인" || val === values?.password,
    },
  ...
  const verifyEmail = useCallback(async (email: string) => {
    await new AuthRepository().sendMail(email).then((response) => {
      response.status === 200 &&
        alert(
          "id로 입력하신 이메일로 인증번호가 발송되었습니다. 확인 후 입력해주세요"
        );
    });
  }, []);
  ...
  const login = useCallback(
    async (id: string, password: string) => {
      try {
        const response = await new AuthRepository().login(id, password);
        setLogin({
    ...
    <>
      {
        authType === "로그인" ? (
          ...
          <SignUpMobile
            ...
            verificationHandler={verifyEmail}
          />
        )
        // <Modal
        //   show={opened}
