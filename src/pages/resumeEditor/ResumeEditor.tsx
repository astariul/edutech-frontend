...
const ResumeBody3Input = ({
  header,
  input1Placeholder,
  input2Placeholder,
  input3Placeholder,
}: ResumeBody3InputProps) => {
  ...
  const removeItem = useCallback(
    (num: number) => {
      const filtered = numAdded.filter((addedNum) => addedNum !== num);
  ...
  return (
    <div style={{ marginTop: "50px" }}>
      <Divider
        size="lg"
        label={<Title order={3}>{header}</Title>}
        labelPosition="center"
      />
      <Button
        className={classes.bodyAddButton}
        variant="subtle"
        onClick={addItem}
      >
        +추가하기
    ...
    <div className={classes.background}>
      <div className={classes.basicInfo}>
        <div>
          ...
          <Divider
            size="lg"
            label={<Title order={3}>기본정보</Title>}
            labelPosition="center"
          />
        </div>
        <div>
